---
name: vibe-planning
description: Use when a non-developer user requests a large or vague feature — anything touching money, user accounts, or data structures; spanning multiple pages/modules; estimated over half a day; or described loosely like "你看着做" / "帮我加个XX系统". Use BEFORE writing any code or any implementation plan. 适用于不会写代码的用户提出较大或较模糊的需求时：加一个系统、做一个模块、想法没想细——在写任何代码之前。
---

# Vibe Planning — 从想法到施工图（6A 工作流）

## 这是什么

不会写代码的用户提一个大需求（"加个会员积分系统，你看着做"），最危险的不是做得慢，而是**AI 按自己的想象把它做完了**。本 skill 把模糊想法变成用户批准过的施工图，然后才允许动工。

它与 vibe-workflow（纪律层）配合：那边管"每件事怎么算做完"，这边管"大事开工前怎么想清楚"。

## 什么算"大需求"（触发判定）

满足任何一条就走本流程，一条不满足就跳过（直接按 vibe-workflow 关卡①对齐后动工）：
...
按 vibe-workflow 关卡③④走：自测全绿（真实环境走流程）→ 台账登记 → 用户验收。
